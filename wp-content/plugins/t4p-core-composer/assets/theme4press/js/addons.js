 ...
 */
(function($) {
	$.t4p_productaddons = function(params) {
		// Object parameters
		this.params = $.extend({
			base_url: '',
			core_plugin: '',
		...
		// Initialize Theme4press addons management
		$(document).ready($.proxy(this.init, this));
	};

	$.t4p_productaddons.prototype = {
		init: function() {
			var self = this;
		...
		request: function(action, identification, authentication, secondary_action) {
			...
			if (action == 'authenticate' && !self.params.has_saved_account) {
				self.authentication.find('.btn-primary').addClass('t4p-loading disabled').attr('disabled', 'disabled').html(self.lang.AUTHENTICATING);
			} else {
				// Toggle status
				switch (action) {
					case 'authenticate':
				...
				addon.addClass('t4p-loading').parent().children('a[data-action]').addClass('disabled').attr('disabled', 'disabled');
			}
			...
			$.ajax({
				...
				if (data.success) {
					...
					if (action == 'authenticate') {
						...
						// Clear status
						addon.removeClass('t4p-loading');

						// Execute secondary action
						self.request(secondary_action, identification, authentication);
					} else {
						// Toggle status
						addon.removeClass('t4p-loading').parent().children('a[data-action]').removeClass('disabled').removeAttr('disabled');
					...
					if (action == 'authenticate') {
						...
						self.authentication.find('.btn-primary').removeClass('t4p-loading').html(self.lang.INSTALL);
					} else {
