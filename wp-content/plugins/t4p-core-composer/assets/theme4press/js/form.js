...
(function($) {
	...
	$.t4p_form.prototype = {
		init: function(event, section) {
			...
			self.accordions = section.find('div[id^="t4p-form-accordion-"]');
			...
			if (self.params.accordions && self.accordions.length) {
				...
				section.find('.t4p-form-fieldset-accordion-toggler').each(function(i, e) {
					var accordion = $(e).closest('fieldset').find('div[id^="t4p-form-accordion-"]');
					...
					self.tabs.on('show.bs.tab', function(event) {
						...
						if (newPanel.html() < 10 && $.trim(newPanel.html()) == '') {
							...
							var	link = window.location.href.replace(/\?section=[0-9a-zA-Z\-_]+/, '').replace('&ajax=1', ''),
								section = newTab.children('a').attr('href').replace('#t4p-form-section-', '');

							// Set loading state for tab panel
							newPanel.addClass('t4p-loading');
							...
							$.ajax({
								...
								complete: function(jqXHR, textStatus) {
									if (textStatus == 'success') {
										var content = jqXHR.responseText.split(new RegExp('<div id="t4p-form-section-' + section + '"[^>]*>'));
										...
										if (content.length > 1) {
											...
											content = content.replace(/\$\(document\)\.ready\(/g, '$(document).on("t4p-document-ready-' + sess + '", ');
											content = content.replace(/\$\(window\)\.load\(/g, '$(window).on("t4p-window-load-' + sess + '", ');

											newPanel.removeClass('t4p-loading').html(content);
											delete content;

											// Trigger necessary events
											$(document).trigger('t4p-document-ready-' + sess);
											$(window).trigger('t4p-window-load-' + sess);
										}
			...
			section.find('.t4p-form-state-switcher, .t4p-form-fieldset-state-switcher').each(function(i, e) {
				$(e).children('.turn-on').unbind('click').bind('click', function(event) {
					var fieldset = $(event.target).parent().hasClass('t4p-form-fieldset-state-switcher') ? $(event.target).closest('fieldset') : section;
		...
		toggle_state: function(fieldset, clicked_element) {
			...
			if (enable) {
				...
				fieldset.find('input[class!="t4p-form-state"], select, textarea, button').each(function(i, e) {
				...
				fieldset.find('a.btn').each(function(i, e) {
					if (
						!$(e).parent().hasClass('t4p-form-state-switcher')
						&&
						!$(e).parent().hasClass('t4p-form-fieldset-state-switcher')
						&&
						!$(e).parent().hasClass('t4p-form-fieldset-accordion-toggler')
					) {
				...
				fieldset.find(self.params.mask_required).css('position', '').children('.t4p-form-disabled-mask').remove();
			} else {
				...
				fieldset.find(self.params.mask_required).css('position', 'relative').append($('<div class="t4p-form-disabled-mask" />'));
			}

			if (clicked_element.parent().hasClass('t4p-form-fieldset-state-switcher')) {
				// Update element contains fieldset state
				fieldset.find('input.t4p-form-state').val(enable ? 1 : 0).trigger('change');
			} else {
