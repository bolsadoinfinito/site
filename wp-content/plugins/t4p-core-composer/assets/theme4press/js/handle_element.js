...
(function($) {
	...
	/**
	 * add Element to T4P PageBuilder when click on an element in Add Elements Popover
	 */
	$.HandleElement.addElement = function() {
		$("body").delegate(".t4p-add-element .shortcode-item","click",function(e) {
			_self(e, this);
		});
		$("#t4p-add-element").delegate(".shortcode-item","click",function(e) {
			_self(e, this);
		});
		...
		function _self(e, this_){
			...
			if($(this_).parents('#t4p-shortcodes').length)
				top.addInClassic = 1;
	...
	$.HandleElement.addItem = function() {
		// Add Item in Modal
		$("body").delegate(".t4p-more-element","click",function(e) {
			_self(e, $(this));
		});
	...
	$.HandleElement.deleteElement = function() {
		$('body').on("click", ".t4p-pb-form-container .element-delete", function(e) {
	...
	$.HandleElement._deleteElement = function(target, silent) {
		...
		if ($(target).hasClass('row') || $(target).attr("data-target") == "row_table") {
			msg = t4p_translate.delete_row;
		} else if ($(target).hasClass('column') || $(target).attr("data-target") == "column_table") {
			msg = t4p_translate.delete_column;
			is_column = 1;
		} else {
			msg = t4p_translate.delete_element;
		}
		...
		if (confirm_) {
			...
			if (is_column == 1) {
				...
				if($(target).attr("data-target") == "column_table") {
					...
					table.deleteColRow($(target), 'column', t4p_translate);
				...
				if ($(target).attr("data-target") == "row_table") {
					table = new $.IGTable();
					table.deleteColRow($(target), 'row', t4p_translate);
					$.HandleSetting.shortcodePreview();
				} else {
					$.HandleElement.removeElement($column, !silent);
				}
	...
	$.HandleElement.appendToHolder = function($shortcode, $replaces, $type, sc_html, elem_title) {
		var append_to_div = $("#form-design-content .t4p-pb-form-container");
		...
		$.post(
			...
			function( data ) {
				if ($replaces && $replaces['index']) {
					data = t4p_pb_remove_placeholder(data, 'index', $replaces['index']);
				}
	...
	$.HandleElement.elTitle = function($shortcode, clk_title_el, exclude_this){
		if(typeof(clk_title_el) == 'undefined' || clk_title_el == '')
			return '';
		var count_element = $(".t4p-pb-form-container").find("a.element-edit[data-shortcode='"+$shortcode+"']").length;
	...
	$.HandleElement.addRawShortcode = function(btn, div) {
		...
		// Toggle adding state
		$(btn).parent().addClass('t4p-loading');
		...
		if (!is_valid) {
			// Toggle adding state
			$(btn).prev('textarea').val('').text('').parent().removeClass('t4p-loading');

			// Reset processing state
			$.PbDoing.addElement = 0;

			return alert(t4p_translate.element_not_existed);
		}
		...
		$.ajax({
			url: t4p_ajax.t4p_modal_url + '&action=insert',
			type: 'POST',
	...
	$.HandleElement.appendToHolderFinish = function($shortcode, html, $replaces, append_to_div, $type, elem_title, position) {
		// Append new element
		if (position) {
			var	rows = $('#t4p_page_builder .jsn-row-container'),
				html = $(html).css({
					display: '',
					height: '',
					opacity: '',
			...
			if ($type != null && $type == 'widget') {
				html = t4p_pb_remove_placeholder(html, 'widget_title', 'title=' + append_title_el);
			} else if (typeof html == 'string') {
				html = html.replace(/el_title=\"\"/, 'el_title="' + append_title_el + '"');
			}

			if ($replaces != null) {
				html = t4p_pb_remove_placeholder(html, 'index', $replaces['index']);
			} else {
				var idx = 0;

				html = t4p_pb_remove_placeholder(html, 'index', function(match, number){
					return ++idx;
				});
			}

			html = $(t4p_pb_remove_placeholder(html, 'custom_style', 'style="display:none"'));
			...
			if (!($shortcode.match(/_item_/) || !append_to_div.hasClass('jsn-element-container'))) {
				...
				$(document).trigger('t4p_pb_after_add_element', html);
			}
	...
	$.HandleElement.appendElementAnimate = function(new_el, height_, callback, finished){
		...
		new_el.animate({
			...
			$(this).animate({
				...
				$('.t4p-pb-form-container').trigger('t4p-pagebuilder-layout-changed');
	...
	/**
	 * Remove an element in T4P PageBuilder / In Modal
	 */
		...
		if (announce) {
			...
			element.animate({
				...
				element.animate({
					...
					$(document).trigger('t4p_pb_before_delete_element', element);

					element.remove();

					// Trigger an event after deleting an element
					$(document).trigger('t4p_pb_after_delete_element');
	...
	$.HandleElement.cloneElement = function() {
		$('body').on("click", ".t4p-pb-form-container .element-clone", function(e) {
			...
			var	parent_item = $(this).parent('.jsn-iconbar').parent('.jsn-item'),
				...
				item_class = $('#modalOptions').length ? '.jsn-item-content' : '.t4p-pb-element';
			...
			if (append_title_el) {
				...
				html = html.replace(regexp, append_title_el + ' ' + t4p_translate.copy);
			}
			...
			$.HandleElement.appendElementAnimate(clone_item, height_, function() {
				...
				$(document).trigger('t4p_pb_after_add_element', [clone_item, 'cloned']);

				if ($('.jsn-modal').length <= 0 && $('.t4p-pb-form-container').hasClass('fullmode')) {
	...
	$.HandleElement.deactivateElement = function() {
		$('body').on("click", ".t4p-pb-form-container .element-deactivate", function(e) {
			...
			if (child_i.hasClass('icon-checkbox-partial')) {
				...
				// Update title
				$(this).attr('title', t4p_translate.disabled.deactivate);
			} else {
				...
				// Update title
				$(this).attr('title', t4p_translate.disabled.reactivate);
			}
			...
			$(document).trigger('t4p_pb_after_edit_element', [parent_item, before]);
	...
	// Edit an Element in T4P PageBuilder / in Modal
	$.HandleElement.editElement = function() {

		// Fix error in element which uses Ajax modal and has child element (Accordion)
		$('body').on("click", ".t4p-dialog", function (e) {
			e.preventDefault();
		});

		$('body').on("click", ".t4p-dialog input:radio, .t4p-dialog input:checkbox", function (e) {
			e.stopPropagation();
		});
		...
		$('body').on("click", "#group_elements .element-edit-sub", function (e) {
			...
			var inline_edit = function( this_settings, custom_action ) {
				...
				$('body').trigger('t4p_submodal_load', [$('#modalOptions')]);

				if (custom_action == 1) {
					var rnd = Math.floor((Math.random() * 100) + 1);
					// Get textarea from ajax response then change its id
					// to create new instance of tinymce
					var new_textarea  = $('.t4p_tinymce_replace textarea', obj.closest('.jsn-item '));
			...
			// Reset sticky item
			$('#group_elements .t4p-sticky-item').attr('style', '');
			$('#group_elements .t4p-sticky-item').attr('class', 'jsn-item-content');
			// Call sticky handle
			...
			if ( ! this_settings.length ) {
				...
				$.ajax({
					url: t4p_ajax.ajaxurl,
					type: 'POST',
					data: { action : 'get_settings_html', shortcode : shortcode, data : shortcode_data, t4p_nonce_check : t4p_ajax._nonce },
					...
					data = t4p_pb_remove_placeholder(data, 'index', $.options.replaces['index']);
		...
		// Edit shortcode
		$('body').on("click", ".t4p-pb-form-container .element-edit", function (e, restart_edit) {
			e.stopPropagation();
...
//            $.HandleElement.removeModal();
            ...
            if ($(this).hasClass('row')) {
            	...
            	parent_item = $(this).parent('.jsn-iconbar').parents('.jsn-column-container').first();
                el_type = 'element';
            } else {
			...
			if (!el_title) {
				el_title = t4p_translate.no_title;
			}
			...
			var frame_url = t4p_ajax.t4p_modal_url + '&t4p_modal_type=' + shortcode;
			...
			var modal = new $.IGModal({
				...
				frameId: frameId,
				dialogClass: 't4p-dialog jsn-bootstrap3',
				jParent : window.parent.jQuery.noConflict(),
				title: $.HandleElement.remove_underscore_ucwords(title),
				///url: t4p_ajax.t4p_modal_url + '&t4p_modal_type=' + shortcode,
				buttons: [{
					'text'  : t4p_ajax.delete,
					'id'    : 'delete_element',
					'class' : 'pull-right',
					'click' : function() {
						if ( $('.jsn-modal').last().attr('id') != $.options.modal_settings.modalId ) {
					...
					'click'	: function() {
						...
						if($.PbDoing.addElement) {
							var active_item = $(".t4p-pb-form-container .active-shortcode").last();
							active_item.remove();
						}
					...
					'click'	: function() {
						...
						$('body').trigger('add_exclude_jsn_item_class');
						$.HandleElement.closeModal(iframe_required ? window.parent.jQuery.noConflict()( '#' + frameId ) : modal.container, iframe_required);
						var cur_shortcode   = $(".t4p-pb-form-container .active-shortcode").last().find('textarea.shortcode-content:first');
						if (cur_shortcode.length > 0) {
				...
				loaded: function (obj, iframe) {
					$('body').trigger('t4p_submodal_load',[iframe]);
					...
					var index = t4p_pb_get_placeholder( 'index' );
					...
					$.ajax({
						...
						complete: function(data, status) {
							if (status == 'success') {
								...
								if ( $('.jsn-modal').last().attr('id') != $.options.modal_settings.modalId ) {
									$('body').trigger('t4p_submodal_load',[modal.container]);
								}
							}
						}
					});
	...
	$.HandleElement.disablePageScroll = function() {
		if ( $('body').hasClass('wp-admin') ) {
			$('body').addClass('t4p-overflow-hidden');
		}
	}
	...
	$.HandleElement.enablePageScroll = function() {
		if ( $('body').hasClass('wp-admin') ) {
			$('body').removeClass('t4p-overflow-hidden');
		}
	}
	...
	$.HandleElement.getModalTitle = function(shortcode, modal_title) {
		var title = t4p_translate.page_modal;
		if (shortcode != '') {
			if(modal_title)
				title = modal_title;
			else{
				shortcode = shortcode.replace('t4p_','').replace('_',' ');
				title = $.HandleElement.capitalize(shortcode);
			}
		}
		return title + ' ' + t4p_translate.settings;
	},
	...
	$.HandleElement.updateBeforeClose = function(update_iframe, rebuild_shortcode, iframe_required) {
		...
		var	shortcode_content = shortcode_content_obj.text(),
			...
			before = $.HandleElement.selector(update_iframe, '.t4p-pb-form-container .active-shortcode').first().outerHTML();
	...
	$.HandleElement.finishCloseModal = function(curr_iframe, update_iframe, before) {
		...
		// Trigger update shortcode for T4P PageBuilder widget element
		$('body').trigger('on_update_shortcode_widget', [shortcode_content]);

		var in_sub_modal = ($('.t4p-dialog').length >= 2);
		...
		if (1) {
			...
			if (contents.find('[data-role="title"]').length) {
				...
				if (title_prepend.length && in_sub_modal) {
					...
					if (typeof(title_prepend_val) != "undefined" && t4p_js_html[title_prepend_type]) {
						if (title_prepend.val() == '' && title_prepend_type == 'icon') {
							title_prepend_val = '';
						} else {
							title_prepend_val = t4p_pb_remove_placeholder(t4p_js_html[title_prepend_type], 'standard_value', title_prepend.val());
						}
					}
				}
			...
			if (contents.find('#t4p-widget-form').length) {
				title_prepend = contents.find('#t4p-widget-form').find("input:text[name$='[title]']");
		...
		if ($.options.if_childmodal) {
			...
			if ($("#modalOptions #shortcode_name").val() == "accordion") {
				$.HandleElement.extractParam("accordion", "tag", "#t4p_share_data");
			}
	...
	$.HandleElement.updateActiveElement = function(update_iframe, shortcode_content, item_title, before) {
		...
		if ( typeof( item_title ) == 'undefined' || ! item_title )
			item_title = t4p_translate.no_title;

		var	active_shortcode = $.HandleElement.selector(update_iframe,".t4p-pb-form-container .active-shortcode").last(),
			before = before || active_shortcode.outerHTML(),
			editted_flag_str = '#_EDITTED';

		active_shortcode = active_shortcode.length ? active_shortcode : $(".t4p-pb-form-container .active-shortcode");

		if (active_shortcode.hasClass('jsn-row-container')) {
			shortcode_content = shortcode_content.replace('[/row]','');
		}
		if (active_shortcode.hasClass('jsn-column-container')) {
			shortcode_content = shortcode_content.replace('[/column]','');
		}
		...
		var item_class = ($.options.if_childmodal) ? ".jsn-item-content" : ".t4p-pb-element";
		...
		if(window.parent.jQuery.noConflict()( '.ui-dialog:last').contents().find('#shortcode_name').val() == "item_table"){
		...
		if (window.parent) {
			window.parent.jQuery(window.parent.document).trigger('t4p_pb_after_edit_element', [active_shortcode, before]);
		} else {
			$(document).trigger('t4p_pb_after_edit_element', [active_shortcode, before]);
		}
		...
		if ($(active_shortcode).parents('.t4p-pb-form-container.fullmode').length > 0) {
	...
	$.HandleElement.finalize = function(is_submodal, remove_modal){
		...
		$(".t4p-pb-form-container").find('.jsn-icon-loading').remove();
		...
		$.options.current_shortcode = 0;
                ...
                // Hide only sub modal in pricing table
                if ( $('#param-prtbl_column .jsn-items-list .jsn-item').attr('data-name') === 'Pricing_column' || $('#param-prtbl_column .jsn-items-list .jsn-item').attr('data-name') === 'Pricing_column Item' ) {
                        is_submodal = '1' ;
                }
	...
	$.HandleElement.convertTo = function(action_data) {
		...
		var active_shortcode = $('.t4p_to_convert');
		var element_html = active_shortcode.html();
		...
		if (arr.length === 3)
		{
			...
			var regexp = new RegExp("" + from_shortcode, "g");
			element_html = element_html.replace(regexp, "" + to_shortcode);

			regexp = new RegExp("" + from_shortcode, "g");
			element_html = element_html.replace(regexp, "" + to_shortcode);
			...
			regexp = new RegExp(t4p_translate.convertText + to_shortcode, "g");
			element_html = element_html.replace(regexp, t4p_translate.convertText + from_shortcode);
		...
		setTimeout(function() {
			...
			active_shortcode.removeClass('t4p_to_convert');
		}, 300);
	}

	$.HandleElement.checkSelectMedia = function() {
		$('body').delegate('#t4p-select-media', 'change', function () {
			var currentValue = $(this).val();
			if ( currentValue ) {
				var jsonObject = JSON.parse( currentValue );
				$('#t4p-select-media').val('');
					...
					wp.media.editor.send.attachment = function(props, attachment){
						if ( _custom_media ) {
							...
							if ( props.size && attachment.type == jsonObject.type) {
								...
								$('#t4p-select-media').val(JSON.stringify(object));
							}
						} else {
	...
	$.HandleElement.initModeSwitcher	= function (){
		...
		var container		= $('.t4p-pb-form-container');
		...
		$('.switchmode-button', switcher_group).on('click', function (){
			...
			if($(this).attr('id')	== 'switchmode-full'){
				...
				container.on('t4p-pagebuilder-layout-changed', function (event, ctn){
					$.HandleElement.switchToFull(ctn);
				});

				container.on('t4p-pagebuilder-column-size-changed', function (event, ctn_row){
				...
				$.HandleElement.setCookie('t4p-pb-mode-' + cur_url, 2);
				...
				container.unbind('t4p-pagebuilder-layout-changed');
				$.HandleElement.setCookie('t4p-pb-mode-' + cur_url, 1)
			}
		});
		// Auto switch to full mode if it was
		if ($.HandleElement.getCookie('t4p-pb-mode-' + cur_url) == 2) {
	...
	$.HandleElement.switchToFull = function (container){
		...
		if ($(container).hasClass('jsn-item') || $(container).parents('jsn-item').length > 0) {
			...
			$('.jsn-element-container .jsn-element.jsn-item', container).each(function (){
				var _shortcode_title	= $('.t4p-pb-element', $(this)).text();
				$(this).find('.t4p-pb-fullmode-shortcode-title').remove();
				$(this).append(
					$("<div/>", {
						"class":"jsn-percent-column t4p-pb-fullmode-shortcode-title"
					}).append(
						$("<div/>", {
	...
	$.HandleElement.turnOnShortcodePreview	= function (shortcode_wrapper){
		...
		if ($(shortcode_wrapper).find('form.shortcode-preview-form').length == 0){
			var _form				= $('<form/>', {
				...
				'action': t4p_ajax.t4p_modal_url + '&t4p_shortcode_preview=1' + '&t4p_shortcode_name=' + _shorcode_name + '&t4p_nonce_check=' + t4p_ajax._nonce
			});
		...
		_iframe.on('load', function (){
			...
			$(this).parents('.jsn-item').find('.t4p-pb-element').hide('slow');
	...
	$.HandleElement.switchToCompact	= function (container){
		...
		$('.t4p-pb-element', container).show();
		$('.t4p-pb-fullmode-shortcode-title', container).remove();
	}

	/**
	 * Update UI of T4P PageBuilder
	 */
	$.HandleElement.updatePageBuilder = function (tab_content, callback){
		...
		$(".t4p-pb-form-container").css('opacity',0);
                $(".t4p-pb-form-container.jsn-layout").hide();
		$("#t4p-pbd-loading").css('display','block');
		if($.trim(tab_content) != ''){
			$.post(
				t4p_ajax.ajaxurl,
				{
		...
		function _self(data){
			// remove current content of T4P PageBuilder
			$("#jsn-add-container").prevAll().remove();

			// insert placeholder text to &lt; and &gt; before prepend, then replace it
			data = t4p_pb_add_placeholder( data, '&lt;', 'wrapper_append', '&{0}lt;');
			data = t4p_pb_add_placeholder( data, '&gt;', 'wrapper_append', '&{0}gt;');
			$(".t4p-pb-form-container").prepend(data);
			$(".t4p-pb-form-container").html(t4p_pb_remove_placeholder($(".t4p-pb-form-container").html(), 'wrapper_append', ''));

			if(callback != null)
				callback();

			// show T4P PageBuilder
			$("#t4p-pbd-loading").hide();
                        $(".t4p-pb-form-container.jsn-layout").show();
			$(".t4p-pb-form-container").animate({
				'opacity':1
			},200,'easeOutCubic');
	...
	$.HandleElement.getContent	= function (){
		var tab_content = '';
		$(".t4p-pb-form-container.jsn-layout textarea[name^='shortcode_content']").each(function(){
			tab_content += $(this).val();
		});
		return tab_content;
	}
	...
	$.HandleElement.deactivateShow = function() {
		...
		$('.shortcode-content').each(function(){
			...
			if (val) {
				...
				deactivate_btn.attr('title', t4p_translate.disabled.reactivate);
	...
	$.HandleElement.initPremadeLayoutAction = function () {
		...
		var frameId = 't4p-layout-lib-modal';
		var modal;

		//----------------------------------- ADD LAYOUT -----------------------------------
		$('#t4p_page_builder #page-template #apply-page').click(function(){
			modal = new $.IGModal({
 				frameId: frameId,
 				dialogClass: 't4p-dialog jsn-bootstrap3',
 				jParent : window.parent.jQuery.noConflict(),
 				title: t4p_translate.layout.modal_title,
 				url: t4p_ajax.t4p_modal_url + '&t4p_layout=1',
 				buttons: [{
 					'text'	: t4p_ajax.cancel,
 					'id'	: 'close',
		...
		$('#t4p_page_builder #page-template #save-as-new').click( function () {
		...
		$('#save-as-new-dialog .template-save').click (function () {
			...
			if (!layout_name) {
				alert(t4p_translate.layout.no_layout_name);
	...
	$.HandleElement.reportBug = function () {
		var frame_id  = 't4p-report-bug-modal';
		var frame_url = t4p_ajax.t4p_modal_url + '&t4p_report_bug=1';
		$('.t4p-report-bug').on('click', function (e) {
			e.preventDefault();
			if ( ! $('#' + frame_id).length ) {
				var report_modal = new $.IGModal({
					...
					dialogClass: 't4p-dialog jsn-bootstrap3',
					jParent: window.parent.jQuery.noConflict(),
					title: t4p_translate.report_bug.modal_title,
					url: frame_url,
					buttons: [{
						'text'  : t4p_translate.report_bug.submit,
						'id'    : 'selected',
						'class' : 'btn btn-primary ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only',
						'click' : function () {
							var jParent = window.parent.jQuery.noConflict();
							var iframe_content = jParent( '#' + frame_id ).contents();
							var data = iframe_content.find('#t4p-form-report-bug').serializeArray();
							...
							$.post(
								...
								function(response) {
									...
									$.HandleElement.enablePageScroll();
									if( response == '1' ) {
										alert( t4p_translate.report_bug.send_mail_success );
									} else{
										alert( t4p_translate.report_bug.send_mail_fail );
									}
								}
							);
						}
	...
	$.HandleElement.bindSelectAttachment = function(jParent) {
		jParent.find('#t4p_attachment').on('change', function () {
			var currentValue = $(this).val();
			if ( currentValue ) {
				var jsonObject = JSON.parse( currentValue );
				jParent.find('#t4p_attachment').val('');
				var send_attachment_bkp = wp.media.editor.send.attachment;
				var button 				= $(this);
					...
					wp.media.editor.send.attachment = function(props, attachment){
						if ( _custom_media ) {
							...
							if ( props.size) {
								...
								object.select_prop	= select_prop;
								object.select_url	= select_url;
								jParent.find('#t4p_attachment').val(select_url);
								jParent.find('#t4p_attachment').attr('value', select_url);
								jParent.find('#t4p_attachment_id').attr('value', attachment.id);
							}
						} else {
							return _orig_send_attachment.apply( this, [props, attachment] );
						};
	...
	$.HandleElement.selectAttachment = function(jParent) {
		...
		jParent.find('.t4p_attachment_remove').on('click', function() {
			var _input	= $(this).closest('div').find('input[type="text"]').first();

			_input.attr('value', '');
			_input.trigger('change');
		});
		jParent.find('.t4p_attachment_select').click(function(e) {
			var	button = $(this),
				object = {};

			jParent.find('#t4p_attachment').val(JSON.stringify(object));
			jParent.find('#t4p_attachment').trigger('change');
		});
	};
	...
	$.HandleElement.customCss = function () {
		// Show modal
		var modal_width = 600;
		var frameId = 't4p-custom-css-modal';
		var modal;

		var post_id = $('#t4p-pb-css-value').find('[name="t4p_pb_post_id"]').val();
		var frame_url = t4p_ajax.t4p_modal_url + '&t4p_custom_css=1' + '&pid=' + post_id;
		...
		$('.jsn-form-bar #page-custom-css').click(function(e) {
			...
			modal = new $.IGModal({
				...
				dialogClass: 't4p-dialog jsn-bootstrap3',
				jParent : window.parent.jQuery.noConflict(),
				title: t4p_translate.custom_css.modal_title,
				url: frame_url,
				buttons: [{
					'text'	: t4p_ajax.save,
					'id'	: 'selected',
					...
					'click'	: function () {
						...
						var css_files = [];
						iframe_content.find('#t4p-pb-custom-css-box').find('.jsn-items-list').find('li').each(function(i){
		...
		if (!document.getElementById('t4p-pb-custom-css-box')) {
			return;
		}
		 ...
		 * Action inside Modal
		 */
		var parent = $('#t4p-pb-custom-css-box');
		...
		parent.find('#items-list-save').click(function(e){
			...
			var custom_css_item_html = $('#tmpl-t4p-custom-css-item').html();
			...
			var hide_file = function(css_files, file) {
				...
				item.parents('li').attr('data-title', t4p_translate.custom_css.file_not_found);
			...
			$.each(valid_urls, function(i, file){
				...
				if (!regex.test(file))
				{
					...
					file_ = t4p_translate.site_url + '/' + file;
				}
	...
	$.HandleElement.stickyItems = function() {
		...
		$('.t4p-pb-setting-tab#content').scroll(function() {
			...
			if (p - start >= (2 + 44 * index)) {
				header.addClass('jsn-element t4p-sticky-item');
				position = 'fixed';
				top = (div.parents('.t4p-pb-setting-tab').offset().top - parseInt(header.css('margin-top').replace('px', ''))) + 'px';
				margin = '-1px';
				height = '45px';
				width = parseInt(header.closest('.jsn-item').width(), 10) + 1 + 'px';
			} else {
				header.removeClass('jsn-element t4p-sticky-item');
			}
