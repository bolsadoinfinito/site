...
/**
 * Root wrapper div .t4p-pb-form-container, contains all content
 * Columns are seperated by a 12px seperators
 * HTML structure: .t4p-pb-form-container [.jsn-row-container [.jsn-column-container]+]+
 *
 */
...
(function ($) {
	...
	JSNLayoutCustomizer.prototype = {
		init:function (_this) {
			...
			this.maxWidth = $('.t4p-pb-form-container.jsn-layout').width();

			// Do this to prevent columns drop
			$('.t4p-pb-form-container.jsn-layout').css('width', this.maxWidth + 'px');
			this.spacing = 12;
			var self	= this;
			...
			this.addColumn($("#tmpl-column").html());
			this.removeItem();
			this.moveItem();
		...
		rebuildSortable: function() {
			...
			$(".t4p-row-content").sortable({
				...
				start: $.proxy(function(event, ui) {
					...
					ui.item.parents(".t4p-row-content").find(".ui-resizable-handle").hide();
				}, this),
				...
				stop: $.proxy(function(event, ui) {
					...
					ui.item.parents(".t4p-row-content").find(".ui-resizable-handle").show();

					// Trigger an event after sorting elements
					if (ui.item[0].__changed) {
						$(document).trigger('t4p_pb_after_edit_element', [ui.item.closest('.jsn-row-container'), ui.item[0].__before]);
					}
					...
					self.wrapper.trigger('t4p-pagebuilder-layout-changed', [ui.item]);
				}, this)
			});

			$(".t4p-row-content").disableSelection();
		...
		resizeHandle:function (self) {
			$(window).resize(function() {
				...
				self.wrapper.trigger('t4p-pagebuilder-column-size-changed', [_rows]);
			});
			$("#t4p_page_builder").resize(function() {
				self.fnReset(self);
			});
		},
		...
		fnReset:function(self, trigger){
			if((self.resize || trigger) && $("#form-design-content").width()){
				// Do this to prevent columns drop
				$(".t4p-pb-form-container.jsn-layout").width($("#form-design-content").width() + 'px');
				self.maxWidth = $(".t4p-pb-form-container.jsn-layout").width();
		...
		calStepWidth:function(countColumn, reset){
			...
			if (reset != null) {
				this_column = $(".t4p-pb-form-container.jsn-layout").find(".jsn-row-container").first().find('.jsn-column-container');
			}
		...
		initResizable: function(countColumn, getStep) {
			...
			var handleResize = $.proxy(function (event, ui) {
				...
				self.wrapper.trigger('t4p-pagebuilder-column-size-changed', [_row]);
			}, this);
			...
			$(".jsn-column").each($.proxy(function (i, e) {
				$(e).resizable({
					...
					stop: $.proxy(function(event, ui) {
						...
						self.resize = 1;
						/// self.updateSpanWidthPBDL(self, self.wrapper, $(".t4p-pb-form-container").width());
						...
						if (ui.element[0].__class.replace(' ui-resizable-resizing', '') != ui.element.attr('class')) {
							$(document).trigger('t4p_pb_after_edit_element', [$([ui.element.parent()[0], ui.element[0].__next.parent()[0]]), ui.element[0].__before]);
						}
		...
		percentColumn:function (element, action,step) {
			...
			if (action == "add") {
				...
				$(element).parents(".jsn-row-container").find(".jsn-column").each(function(){
					if(!$(this).find(".t4p-pb-layout-percent-column").length){
						var thisCol = this;
			...
			if (action == "remove") {
				...
				$(container).find(".t4p-pb-layout-percent-column").remove();
			}
		},
		...
		showPercentColumn:function(thisCol, thisSpan){
			...
			if ($(thumbnail).find(".t4p-pb-layout-percent-column").length) {
				$(thumbnail).find(".t4p-pb-layout-percent-column .jsn-percent-inner").html(percent);
			} else {
				$(thumbnail).append(
					$("<div/>", {"class":"jsn-percent-column t4p-pb-layout-percent-column"}).append(
			...
			var widthPercent = $(thumbnail).find(".t4p-pb-layout-percent-column").width();
			$(thumbnail).find(".t4p-pb-layout-percent-column").css({"left":parseInt((widthThumbnail + 10) / 2) - parseInt(widthPercent / 2) + "px"});
			$(thumbnail).find(".t4p-pb-layout-percent-column .jsn-percent-arrow").css({"left":parseInt(widthPercent / 2) - 4 + "px"});
		},
		...
		addRow: function(self, this_wrapper){
			...
			this.wrapper.delegate('.t4p-layout-thumbs .thumb-wrapper', 'click', function(event) {
			...
			this.wrapper.delegate('#jsn-add-container', 'mouseover', function(event) {
				$('.t4p-layout-thumbs').addClass('t4p-mouse-over');
				var timeout = setTimeout(function () {
					if ( $('.t4p-layout-thumbs').hasClass('t4p-mouse-over') && ! $('.t4p-layout-thumbs').hasClass('open') ) {
						$('.t4p-layout-thumbs').addClass('open');
						$('.t4p-layout-thumbs').css({'height': 'auto', 'max-height': '150px'});
					}
			...
			this.wrapper.delegate('#jsn-add-container', 'mouseleave', function(e) {
				e.preventDefault();
				$('.t4p-layout-thumbs').removeClass('t4p-mouse-over');
			});

			$('#form-design-content').on('mouseleave', function (e) {
				e.preventDefault();
				$('.t4p-layout-thumbs').removeClass('open');
				$('.t4p-layout-thumbs').css({'max-height': '1px'});
			});
		...
		_addRow: function(this_wrapper, target, row_html, position, get_chosen_layout) {
			...
			if ($('.t4p-pb-form-container.jsn-layout').find('.jsn-row-container').last().is(':animated')) return;
			...
			if (row_html && position) {
				...
				row_html = $(t4p_pb_remove_placeholder($('#tmpl-row').html(), 'custom_style', 'style="display:none"'));
				var full_row_html = row_html.find('.t4p-row-content').html();

				var html = '';
				if( get_chosen_layout && $('.t4p-layout-thumbs .active').length > 0 ) {
					var columns = $('.t4p-layout-thumbs .active').attr('data-columns');
				...
				if ( html !== '' )
					row_html.find('.t4p-row-content').html( html );

				$(target).before(row_html);

				// Trigger an event after adding an element
				$(document).trigger('t4p_pb_after_add_element', row_html);
			...
			self.updateSpanWidthPBDL(self, self.wrapper, $(".t4p-pb-form-container.jsn-layout").width());
		},
		...
		swapContentRow: function(a, b, direction) {
			...
			if (direction > 0) {
				...
				b.animate({top: '' + (stylea.height + extra2) + 'px'}, time, effect, function() {
					...
					$(document).trigger('t4p_pb_after_edit_element', [$([b[0], a[0]]), before]);
				});
			} else {
		...
		moveItem:function(){
			...
			this.wrapper.delegate(this.moveItemEl, "click",function(){
				if(!$(this).hasClass("disabled")){
					...
					setTimeout(function (){
						self.wrapper.trigger('t4p-pagebuilder-layout-changed', [parent]);
					}, 1001);
		...
		_addColumn: function(column_html, target, position) {
				...
				for (var i = 0; i < rows.length; i++) {
					if (i == position.row) {
						...
						if (columns.length) {
							...
							position = parentForm.find('.t4p-row-content');
						}
			...
			if (countColumn < 12) {
				...
				// Add new column
				var rowContent = parentForm.find(".t4p-row-content");
				...
				if (typeof position != 'undefined') {
					...
					// Trigger an event after adding an element
					$(document).trigger('t4p_pb_after_add_element', column_html);
				}
			...
			self.rebuildSortable();
			self.wrapper.trigger('t4p-pagebuilder-layout-changed', [parentForm]);
		},
		...
		sortableElement: function() {
			...
			$(".jsn-element-container").sortable({
				...
				stop: function(event, ui) {
					// Trigger an event after sorting elements
					if (ui.item[0].__changed) {
						$(document).trigger('t4p_pb_after_edit_element', [$(ui.item[0].__after), ui.item[0].__before]);
					}
		...
		showPopover:function(box, e, self, this_, callback1, callback2){
			...
			var row_idx= $(".t4p-pb-form-container.jsn-layout .jsn-row-container").index(this_.parents('.jsn-row-container'));
		...
		addElement:function(){
			...
			this.wrapper.delegate(this.addelements,"click",function(e){
				...
				var frame_url = t4p_ajax.t4p_modal_url + '&t4p_add_element=1';
				var report_modal = new $.IGModal({
					dialogClass: 't4p-dialog jsn-bootstrap3',
					jParent: window.parent.jQuery.noConflict(),
					title: t4p_translate.select_element_title,
					url: frame_url,
					loaded: function (obj, iframe) {
						if ( $('#t4p-add-element-modal').length == 0 ) {
							report_modal.container.attr('id', 't4p-add-element-modal');
						}
						...
						var html_box = $("<div />").append('<div id="t4p-shortcodes" class="t4p-add-element add-field-dialog jsn-bootstrap3">' + html_box.html() + '</div>').html();
						$("#t4p-add-element").remove();
						...
						var box = $("#t4p-add-element");
						$(box).show();

						$.HandleCommon.setFilterFields('.t4p-add-element:last');
				        $.HandleCommon.setQuickSearchFields('.t4p-add-element:last');

						self.updateSpanWidthPBDL(self, self.wrapper, $(".t4p-pb-form-container.jsn-layout").width());
						self.updateSpanWidthPBDL(self, self.wrapper, $(".t4p-pb-form-container.jsn-layout").width());

						$(window).on('resize', function () {
							self.reCalculateSize('.t4p-dialog');
						});
					},
					fadeIn:200,
					scrollable: true,
		...
		reCalculateSize: function (box) {
			if ( $('.jsn-modal:last').attr('id') == 't4p-add-element-modal' ) {
				var width = $.HandleElement.resetModalSize(0, 'w');
				var height = $(window.parent).height() * 0.8;
				$(box).width(width);
				$(box).height(height);
				$('#t4p-add-element-modal').height('100%');
		...
		showSaveLayoutBox:function(){
			$('#t4p-pb-save-layout').click(function(e){
				$(this).toggle();
				$('#t4p-pb-layout-form').toggleClass('hidden');
			});
		},
		...
		searchElement:function(){
			var self = this;
			$.HandleCommon.setQuickSearchFields('#t4p-add-element');
		},
		...
		filterElement:function(value, filter_data){
			var resultsFilter = $('#t4p-add-element .jsn-items-list');
			if (value != "all") {
	...
	$.HandleCommon.removeConfirmMsg = function(item, type, column_to_row, callback, silent) {
		...
		if (confirm_) {
			if (!silent) {
				...
				$(document).trigger('t4p_pb_before_delete_element', item);
			}
			...
			if (silent) {
				...
				if (type == 'row') {
					JSNLayoutCustomizer.prototype.moveItemDisable($(".t4p-pb-form-container.jsn-layout"));
				} else if (type == 'column') {
					if (callback != null) {
						callback();
					}
	...
	$(document).ready(function() {
		// T4P PageBuilder: get textarea content
		var t4p_pg_html = '';
		// T4P PageBuilder: check if content has changed
		...
		var text_content = $('#t4p_editor_tab1 #content').val();

		// Trigger when Text tab of Classic Editor is changed
		$('#t4p_editor_tab1 #content').bind('input propertychange', function(){
			_change_editor = 1;
			text_content = $(this).val();
		});

		// Switch tab: Classic editor , T4P PageBuilder
		$('#t4p_editor_tabs a').click(function (e, init_layout) {
			...
			e.preventDefault();
                        ...
                        if ( hash == "#t4p_editor_tab1" ) {
                            //pagebuilder -> classic
                            
//                            if ( ! confirm ("Would you like to clear your Page Builder content and revert to using the classic editor?") ) {
//                                $("#t4p_active_tab").val(1);
//
//                                $('#t4p_editor_tabs a').parent().addClass('active');
//                                if( $(this).parent().attr('data-id') == 'classic' ) {
//                                        $(this).parent().removeClass('active');
//                                }
//
//                                $('#t4p_editor_tab2').addClass('active');
//
//                                $('#t4p_editor_tabs a').parent().removeClass('pagebuilder');
//                                if( $(this).parent().attr('data-id') == 'classic' ) {
//                                        $(this).parent().addClass('pagebuilder');
//                                }
//
//                                return false;
//                            }
                                if ($(this).parent().hasClass('classiceditor')) {
                                    alert ("Would you like to clear your Page Builder content and revert to using the classic editor?");
                                }
                                $(this).parent('li').removeClass('classiceditor');
                                $('#t4p_editor_tabs a').parent('li').addClass('pagebuilder');
                                if( $(this).parent().attr('data-id') == 'classic' ) {
                                        $(this).parent().removeClass('pagebuilder');
                                }
                        } else if ( hash == "#t4p_editor_tab2" ) {
                            //classic -> pagebuilder
                                if ($(this).parent().hasClass('pagebuilder')) {
                                    alert ("Would you like to copy this editor's existing content to Page Builder?");
                                }
                                $(this).parent('li').removeClass('pagebuilder');
                                $('#t4p_editor_tabs a').parent('li').addClass('classiceditor');
                                if( $(this).parent().attr('data-id') == 'pagebuilder' ) {
                                        $(this).parent().removeClass('classiceditor');
                                }
                        }
			...
			// Init T4P PageBuilder
			if(this.hash == '#t4p_editor_tab2' && ! $.t4p_pb_layout){
				$.t4p_pb_layout = new JSNLayoutCustomizer();
				$.t4p_pb_layout.init($(".t4p-pb-form-container.jsn-layout .jsn-row-container"));
			}
			...
			// T4P PageBuilder is deactivate, stop processing
			...
			switch (hash) {
				...
				// T4P PageBuilder -> Classic Editor
				case '#t4p_editor_tab1':
					// Check if content of T4P PageBuilder is changed
					var old_t4p_pg_html = t4p_pg_html;

					// Get Raw shortcodes content in T4P PageBuilder tab
					var t4p_pb_textarea_content = $('.t4p-pb-form-container.jsn-layout textarea').serialize();

					// Update value for t4p_pg_html
					if(t4p_pg_html == '' || t4p_pg_html != t4p_pb_textarea_content){
						t4p_pg_html = t4p_pb_textarea_content;
					}

					// Check if value is changed
					if(old_t4p_pg_html != t4p_pg_html){
						_change_pagebuilder = 1;
					}

					// Check if content of 2 tabs are same
					if($("#t4p_editor_tab1 #content").val() == tab_content) {
						_change_pagebuilder = 0;
					}

					// Get nice shortcodes content in T4P PageBuilder tab
					$(".t4p-pb-form-container.jsn-layout textarea[name^='shortcode_content']").each(function(){
						tab_content += $(this).val();
					});

					// Synchronize content of classic editor with T4P PageBuilder
					var text_content_ = ( window.tinymce && tinymce.get('content') ) ? tinymce.get('content').getContent() : $('#t4p_editor_tab1 #content').val();
					...
					if(_change_pagebuilder){
						...
						tab_content = t4p_pb_remove_placeholder(tab_content, 'wrapper_append', '');
				...
				// Classic Editor -> T4P PageBuilder
				case '#t4p_editor_tab2':
					...
					// Synchronize content of classic editor with T4P PageBuilder
					tinyMCE.triggerSave();
					text_content = $('#t4p_editor_tab1 #content').val();

					// Remove Shortcode content which is not wrapped in row & column
					var remove_div = $('.t4p-pb-form-container.jsn-layout #t4p-select-media').nextAll( ".jsn-item" );
					...
					if(_change_editor){
						...
						// Update T4P PageBuilder content
						$.HandleElement.updatePageBuilder(tab_content, function(){
							// Reset T4P PageBuilder Layout manager
							$.t4p_pb_layout.fnReset($.t4p_pb_layout, true);
							$.t4p_pb_layout.moveItemDisable($.t4p_pb_layout.wrapper);
						});
