...
(function($) {
	...
	$.t4p_image_element.setImageSize	= $.t4p_image_element.setImageSize || {};
	...
	$.HandleSetting.buttonGroup = function() {
		...
		$('.t4p-btn-group .btn').click(function(i) {
			data_value = $(this).attr('data-value');

			$(this).parent().next('.t4p-btn-radio').find('input:radio[value="'+data_value+'"]').prop('checked', true);
	...
	$.HandleSetting.inputValidator = function() {
		...
		$("#param-el_title, input:text[name$='[title]'], [data-role='title'], .t4p-pb-limit-length", '#modalOptions').each(function() {
	...
	$.HandleSetting.selectImage = function() {
		...
		$('#modalOptions .select-media-remove').on('click', function() {
			...
			_input.trigger('change');
                ...
                $('body').on( 'click', '#modalOptions .select-media-remove', function(e) {
                        var _input	= $(this).closest('div').find('input[type="text"]');
		...
		$('body').on( 'click', '#modalOptions .select-media', function(e) {
			...
			jqueryParent('#t4p-select-media').val(JSON.stringify(object));
			jqueryParent('#t4p-select-media').trigger('change');

			var timer = setInterval(function() {
				var currentValue = jqueryParent('#t4p-select-media').val();
				...
				if (currentValue) {
					...
					switch ( jsonObject.type ) {
						case 'media_selected':
							if (typeof($.t4p_image_element.setImageSize) == 'function') {
								$.t4p_image_element.setImageSize(jsonObject.select_prop);
							}
		...
		$(selector).each(function() {
			...
			self.ColorPicker({
				...
				onHide: function(colpkr) {
					...
					if ($(".color-selector").parent().parent().parent().parent().hasClass('sub-element-settings') || 
                                                $(".color-selector").parent().parent().parent().hasClass('sub-element-settings') 
                                                    ) {
	...
	$.HandleSetting.shortcodePreview = function(params, shortcode, curr_iframe, callback, stop_reload_iframe, child_element, selector_group) {
		...
		if (params == null) {
			...
			shortcode_type = $.HandleSetting.selector(curr_iframe, '.t4p-pb-form-container #shortcode_type' ).val();

			if (shortcode_type == 'widget') {
				// Widget
				var form_serialize = $.HandleSetting.selector(curr_iframe, '#modalOptions #t4p-widget-form' ).serialize();

				$('input[type=checkbox]', '#modalOptions #t4p-widget-form').each(function() {
					if (!this.checked) {
						form_serialize += '&'+this.name+'=0';
					}
				});

				tmp_content = '[widget widget_id="'+shortcode_name+'"]' + form_serialize + '[/widget]';
			} else {
				...
				// Update TinyMCE content to #t4p_share_data
				window.parent.jQuery.noConflict()('#jsn_view_modal').contents().find('#t4p_share_data').text(sc_content);
				...
				// For shortcode which has sub-shortcode
					...
					$.HandleSetting.selector(curr_iframe, "#modalOptions [name^='shortcode_content']").each(function() {
						...
						$('#modalOptions').trigger('t4p_get_sub_sc', [obj]);
                                                if (obj.sc_content != '') {
		...
		var url = t4p_ajax.t4p_modal_url + '&t4p_shortcode_preview=1' + '&t4p_shortcode_name=' + shortcode_name + '&t4p_nonce_check=' + t4p_ajax._nonce;
    ...
    $.HandleSetting.traverseParam = function( $selector, child_element, get_data_tag ){
        ...
        $selector.each( function ()
        {
            if ( ! $(this).hasClass( 't4p_hidden_depend' ) )
            {
                $(this).find( '[id^="param-"]' ).each(function()
                {
                    ...
                    if(
						...
						&& ! $(this).parents("[id^='parent-param']").hasClass( 't4p_hidden_depend' )
                    ...
                    )
                    {
                        ...
                        if( get_data_tag == null || get_data_tag ) {
                            ...
                            if($(this).parent().hasClass('merge-data')){
                                var t4p_merge_data = window.parent.jQuery.noConflict()( '#jsn_view_modal').contents().find('#t4p_merge_data');
                                t4p_merge_data.text(t4p_merge_data.text() + $(this).val());
                            }

                            // table
                            if($(this).attr("data-role") == "extract"){
                                var extract_holder = window.parent.jQuery.noConflict()( '#jsn_view_modal').contents().find('#t4p_extract_data');
    ...
    $.HandleSetting.generateShortcodeContent = function(shortcode_name, params_arr, sc_content){
        if ( shortcode_name == 'column' ) {
                var params = $('#column-span' ).attr('data-name');
        }
        var tmp_content = [];
        //var shortcode_name = shortcode_name.replace('t4p_', '');
        tmp_content.push('['+ shortcode_name);
        if ( shortcode_name == 'column' ) {
                tmp_content.push('span="'+params+'"');
        }
	...
	$.HandleSetting.loadIframe = function(curr_iframe, url, tmp_content) {
		$('#t4p_preview_data').remove();

		var tmp_form = $(
			'<form action="' + url + '" id="t4p_preview_data" name="t4p_preview_data" method="post" target="shortcode_preview_iframe">' +
				'<input type="hidden" id="t4p_preview_params" name="params" value="' + encodeURIComponent(tmp_content) + '">' +
			'</form>'
		).appendTo($('body'));

		$.HandleSetting.selector(curr_iframe, '#modalOptions #t4p_overlay_loading').fadeIn('fast');

		$('#t4p_preview_data').submit();

		$('#modalOptions #shortcode_preview_iframe').bind('load', function() {
			$('#modalOptions #shortcode_preview_iframe').fadeIn('fast');

			$.HandleSetting.selector(curr_iframe, '#modalOptions #t4p_overlay_loading').fadeOut('fast');

			$('#t4p_previewing').val('0');
	...
	$.HandleSetting.toggleDependencyAll = function() {
		...
		var t4p_HasDepend = $( '#modalOptions .t4p_has_depend' );

		t4p_HasDepend.each(function() {
			if (($(this).is(":radio") || $(this).is(":checkbox")) && !$(this).is(":checked")) {
				return;
			}
	...
	$.HandleSetting.toggleDependency = function(this_id, this_val) {
		...
		$('#modalOptions .t4p_depend_other[data-depend-element="' + this_id + '"]').each(function() {
			...
			switch(operator) {
				case "=":
					...
					if (check_) {
						$(this).removeClass('t4p_hidden_depend');
					} else {
						$(this).addClass( 't4p_hidden_depend' );
					}
				break;
			...
			$.HandleSetting.secondDependency($(this).attr('id'), $(this).hasClass('t4p_hidden_depend'), $(this).find('select').hasClass('no_plus_depend'));
		});
	};
	...
	$.HandleSetting.secondDependency = function(this_id, hidden, allow) {
		...
		$('#modalOptions .t4p_depend_other[data-depend-element="'+this_id+'"]').each(function() {
			if (hidden) {
				$(this).addClass('t4p_hidden_depend2');
			} else {
				$(this).removeClass('t4p_hidden_depend2');
			}
		});
		...
		// Hide label if all options in .controls div have 't4p_hidden_depend' class
	...
	$.HandleSetting.tab = function() {
		if ($('.jsn-tabs').length && !$('.jsn-tabs').find("#Notab").length) {
			...
			$('#t4p_option_tab a[data-toggle="tab"]').on('click', function() {
			...
			if ($('#t4p_previewing').val() == '1') {
				return;
			}
			$('#t4p_previewing').val('1');
	...
	$.HandleSetting.select2_color = function() {
		function format(state) {
			...
			return "<img class='color_select2_item' src='" + t4p_translate.asset_url + "images/icons-16/btn-color/" + type + ".png'/>" + state.text;
		}
	...
	$.HandleSetting.icons = function() {
		...
		$('body').on("click", ".t4p-pb-form-container [data-type='t4p-icon-item']", function() {
	...
	$.HandleSetting.actionHandle = function() {
		...
		$('body').on("click", ".t4p-pb-form-container .t4p_action_btn a", function(e) {
			e.preventDefault();
	...
	$.HandleSetting.copyToClipboard = function() {
		...
		if (textarea.length && button.length) {
			ZeroClipboard.config({ moviePath: t4p_ajax.assets_url + '/assets/3rd-party/zeroclipboard/ZeroClipboard.swf' });
	...
	$.HandleSetting.initCopyStyle = function () {
		...
		copy_button.on('click', function () {
			...
			if (typeof(t4p_pb_modal_ajax) != 'undefined') {
				if (t4p_pb_modal_ajax == 1) {
						...
						$('#frm_shortcode_settings #hid-params').val(data);
							if (typeof(t4p_pb_modal_ajax) != 'undefined') {
								if (t4p_pb_modal_ajax == 1) {
									...
									$.ajax({
										url: t4p_ajax.t4p_modal_url + '&t4p_modal_type=' + shortcode_name + '&form_only=1',
	...
	$.HandleSetting.handleResizeModal = function () {
		if ( typeof ( $('.t4p-setting-resize').resizable) == 'function' ) {
			$('.t4p-setting-resize').resizable({
			    handles: 'e',
			    minWidth: 400,
			    start: $.proxy(function (event, ui) {
			    	$('.t4p-preview-resize #t4p_overlay_loading').show();
			    }, this),
				resize: $.proxy(function (event, ui) {
					...
					$('.t4p-preview-resize').css('width', remain_width + 'px');
				}, this),
				stop: $.proxy(function (event, ui) {
					$('.t4p-preview-resize #t4p_overlay_loading').fadeOut();
				}),
			});
		}
		...
		if ( $('.jsn-modal').last().attr('id') == 'jsn_view_modal') {
			setTimeout( function() {
				$('.t4p-setting-resize').css( { 'width': '33%' } );
				$('.t4p-preview-resize').css( { 'width': '65%' } );
			}, 100 );
		}

		$('.jsn-modal').on('resize', function () {
			if ( $('.jsn-modal').last().attr('id') == 'jsn_view_modal') {
				if ( $('.t4p-preview-resize').length ) {
					$('#jsn_view_modal').css({'overflow': 'hidden'});
				}
				var modal_width = $('#modalOptions').width();
				var panel_width = $('.t4p-setting-resize').first().width();
				$('.t4p-preview-resize').width( modal_width - panel_width - 18 );
				var modal_content_height = $('#jsn_view_modal').height();

				// Fixed size for preview/settings panels.
				$('#jsn_view_modal #modalOptions .t4p-setting-resize .tab-pane').height( modal_content_height - 70 );
	...
	$.HandleSetting.useBootstrapJS = function () {
		// For dropdown
		$('.t4p-dropdown-toggle').each(function () {
	...
	$.HandleSetting.addTooltip = function () {
		if ( typeof ( $.fn.tooltip ) == 'function' ) {
			$('.t4p-tooltip-toggle').tooltip({
				placement: 'bottom'
			});
			$('[data-original-title]').not('.t4p-tooltip-toggle').tooltip({
				placement: 'bottom'
			});
		}
	}
	...
	$.HandleSetting.addStyleBorderType = function () {
		if ( typeof( $.fn.select2 ) == 'function' ) {
			var format = function ( state ) {
				...
				if ( ! border_type )
					border_type = 'solid';
				return state.text + "<div class='t4p-border-line' style='border-top: 4px " + border_type + " #C3C3C3'></div>";
			};
			$('.t4p-border-type').select2('destroy');
			$('.t4p-border-type').select2({
				formatResult: format,
				formatSelection: format,
	...
	$.HandleSetting.addStyleHeadingType = function () {
		if ( typeof( $.fn.select2 ) == 'function' ) {
			...
			$('.t4p-heading-type:last').select2('destroy').select2({
				formatResult: format,
				//formatSelection: format,
				escapeMarkup: function (m) { return m; }
			});
	...
	$.HandleSetting.addSelectEditor = function () {
		if ( typeof( $.fn.select2 ) == 'function' ) {
			...
			var onSelectionItem = function ( current_id ) {
				var new_term = $('.select2-results:last .t4p-new-term').text();
			...
			$('.t4p-select2-editor').each(function () {
				var current_id = $(this).attr('id');
				$(this).select2({
				    allowClear:true,
				    formatNoMatches: function(term) {
				    	term = parseInt( term );
				    	return "<div class='select2-result-label t4p-new-term'><span class='select2-match'>"+term+"</span></div>";
				    },
				}).parent().find('.select2-with-searchbox').on('keyup', function(e) {
					if(e.keyCode === 13) {
						onSelectionItem( current_id );
					}
				}).on( 'click','.t4p-new-term',function(){
					onSelectionItem( current_id );
				} );
			});
		}
	...
	$.HandleSetting.init = function() {
		...
		$.HandleSetting.changeDependency('.t4p_has_depend');

		// Update preview when change param in Modal Box/Styling
		$('#t4p-element-pricing_column #modalOptions, #modalOptions').delegate('[id^="param"]', 'change', function() {
			...
			//var	parent_tab = $(this).parents('.t4p-pb-setting-tab'),
		...
		$('#t4p-element-pricing_column #modalOptions, #modalOptions').on( 'change', '.sub-element-settings [id^="param"]', function(e) {
			e.stopPropagation();
			$.HandleSetting.shortcodePreview(null, null, null, null, 0, 1, $('.control-group', '.sub-element-settings:visible'));
		} );

		// Trigger preview for parameters of Widget
		$('#modalOptions #t4p-widget-form').delegate('input, select, textarea', 'change', function() {
			$.HandleSetting.shortcodePreview();
		});

		if ($('#shortcode_name').val() != 't4p_image') {
