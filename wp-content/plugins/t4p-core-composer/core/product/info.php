...
if ( ! class_exists( 'T4P_Pb_Product_Info' ) ) :
...
class T4P_Pb_Product_Info {
	...
	/**
	 * Hook into Theme4Press' products.
	 *
	 * @return  void
	 */
	public static function hook() {
		...
		if ( ! isset( $registered ) ) {
			add_action( 't4p_envato_purchase_data', array( __CLASS__, 'save' ), 10, 2 );
	 ...
	 * array(
	 *     'Name'             => 'T4P Sample',
	 *     'Description'      => 'Sample plugin that demonstrates the functionality of T4P Library (Theme4Press's shared library). By theme4press.com.',
	 *     'Version'          => '1.0.0',
	 *     'Item_ID'  => 't4p-sample',
	 *     'Addons'           => null,
	 *     'Available_Update' => 0,
	...
	public static function save( $plugin, $options ) {
		...
		if ( ! empty( $plugin ) ) {
			...
			// Request Theme4Press to verify purchase data
			$slug = apply_filters( 't4p_product_update_slug', basename( dirname( $plugin['Main_File'] ) ) );
	 ...
	 *
	 * @param   string   $name      Plugin's identified name defined in Theme4Press server.
	...
	protected static function info( $item_id ) {
		...
		if ( ! isset( self::$product_data[ $item_id ] ) ) {
			self::$product_data[ $item_id ] = get_transient( "t4p_product_info_{$item_id}" );
			...
			if ( ! self::$product_data[ $item_id ] ) {
				...
				if ( ! is_wp_error( $result ) ) {
					if ( self::$product_data[ $item_id ] = json_decode( $result['body'], true ) ) {
						// Store data to cache
						set_transient( "t4p_product_info_{$item_id}", self::$product_data[ $item_id ], self::$cache_time );
					}
				}
			}
