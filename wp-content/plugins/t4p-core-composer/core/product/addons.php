...
if ( ! class_exists( 'T4P_Pb_Product_Addons' ) ) :
 ...
 * @since    1.0.0
 */
class T4P_Pb_Product_Addons extends T4P_Pb_Product_Info {
	/**
	...
	public static function hook() {
		...
		if ( ! isset( $registered ) ) {
			add_action( 'wp_ajax_t4p-addons-management',  array( __CLASS__, 'manage' ) );
	 ...
	 *
	 * @param   string  $plugin_file  Either absolute path to plugin main file or plugin's identified name defined in Theme4Press server.
	 *
	 * @return  void
	 */
	public static function init( $plugin_file ) {
		...
		// Get template path
		if ( $tmpl = T4P_Pb_Loader::get_path( 'product/tmpl/addons.php' ) ) {
			// Get product information
			$plugin = self::get( $plugin_file );

			if ( ! $plugin ) {
				die( __( 'Cannot get addons information for current product.', 't4p-core' ) );
			}

			// Check if user has customer account saved
			$customer_account     = get_option( 't4p_customer_account', null );
	...
	public static function manage() {
		...
		try {
			...
			if ( empty( $addon ) ) {
				throw new Exception( __( 'Missing addon to be installed.', 't4p-core' ) );
			}
			...
			if ( empty( $action ) || ! array_key_exists( $action, self::$actions ) ) {
				throw new Exception( __( 'The requested action is invalid.', 't4p-core' ) );
			}
	...
	protected static function execute( $addon, $action ) {
		...
		if ( 'update' == $action || 'uninstall' == $action ) {
			...
			if ( empty( $plugin ) ) {
				throw new Exception(
					__( 'update' == $action ? 'Cannot detect plugin to be updated.' : 'Cannot detect plugin to be removed.', 't4p-core' )
				);
			}
		}
		...
		if ( 'uninstall' == $action ) {
			...
			$wp_filesystem = T4P_Pb_Init_File_System::get_instance();
			...
			if ( 'authenticate' != $action ) {
				...
				if ( isset( $_GET['core'] ) && ( $core = self::get( $_GET['core'] ) ) ) {
					...
					if ( $core_version && version_compare( $core_version, $core['Version'], '>' ) ) {
						...
						throw new Exception(
							sprintf(
								__( "Cannot install %1\$s v%2\$s.\nThis version requires %3\$s v%4\$s while you are using %5\$s v%6\$s.", 't4p-core' ),
				...
				if ( isset( $_POST['remember'] ) && (boolean) $_POST['remember'] ) {
					update_option( 't4p_customer_account', array( 'username' => $username, 'password' => $password ) );
				}
			}
		}
	 ...
	 *
	 * @param   string   $addon              Identified name of add-on (as defined in Theme4Press server).
	...
	protected static function download( $addon, $authentication = false, $username = null, $password = null, $authenticate_only = false ) {
		...
		if ( $wp_filesystem->size( $result ) < 10 ) {
			...
			switch ( $content ) {
				case 'ERR00':
					throw new Exception( __( 'Invalid Parameters! Can not verify your product information.', 't4p-core' ) );
				break;

				case 'ERR01':
					throw new Exception( __( 'Invalid username or password.', 't4p-core' ) );
				break;

				case 'ERR02':
					throw new Exception( __( 'We could not find the product in your order list. Seems like you did not purchase it yet.', 't4p-core' ) );
				break;

				case 'ERR03':
					throw new Exception( __( 'Requested file is not found on server.', 't4p-core' ) );
				break;

				default:
					if ( 'true' != $content && 'yes' != $content && '1' != $content ) {
						throw new Exception( __( 'Cannot connect to Theme4Press.com server.', 't4p-core' ) );
					}
				break;
			}
		}
		...
		if ( ! $authenticate_only ) {
			if ( ! $wp_filesystem->copy( $result, $target, true, FS_CHMOD_FILE ) ) {
				...
				if ( ! $wp_filesystem->copy( $result, $target, true, FS_CHMOD_FILE ) ) {
					throw new Exception( __( 'Cannot store add-on package to local file system.', 't4p-core' ) );
				}
			}
		}
	...
	protected static function activate( $addon, $action = '' ) {
		...
		if ( empty( $plugin ) ) {
			throw new Exception( __( 'Cannot detect plugin to activate.', 't4p-core' ) );
		}
