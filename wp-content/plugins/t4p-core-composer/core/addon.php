...
class T4P_Pb_Addon {
	...
	/**
	 * Get custom enqueued assets for T4P modal
	 *
	 * @return type
	 */
	...
	/**
	 * Add custom assets for T4P modal
	 *
	 * @param array $assets
	 */
	...
	public function __construct() {
		add_filter( 't4p_pb_provider', array( &$this, 'get_provider_data' ) );
		add_filter( 't4p_pb_register_assets', array( &$this, 'register_assets_register' ) );
		add_filter( 't4p_pb_assets_enqueue_admin', array( &$this, 'enqueue_assets_admin' ) );
		add_filter( 't4p_pb_assets_enqueue_modal', array( &$this, 'enqueue_assets_modal' ) );
		add_filter( 't4p_pb_assets_enqueue_frontend', array( &$this, 'enqueue_assets_frontend' ) );
	}
	...
	/**
	 * Register custom assets for T4P modal
	 *
	 * @param array $assets
	 *
	 * @return array
	...
	public function register_extended_parameter_path( $path ) {
		T4P_Pb_Loader::register( $path, 'T4P_Pb_Helper_Html_' );
	}
...
<div class="<?php echo esc_attr( $type ); ?>">
	<p>
	<?php _e( "You can not activate this T4P PageBuilder's provider:", 't4p-core' ); ?>
		<br> <b><?php echo esc_html( $addon_name ); ?> </b>
	</p>

	<p>
	<?php _e( "It requires T4P PageBuilder's version:", 't4p-core' ); ?>
		<br> <b><?php echo esc_html( $core_required ); ?> </b> <br>
		<?php echo esc_html( 'or above to work. Please update T4P PageBuilder to newest version.' ); ?>
		<br>
	</p>
</div>
...
<!-- custom js to hide "Plugin actived" -->
		...
		echo balanceTags( T4P_Pb_Helper_Functions::script_box( $js_code ) );
...
default:
	...
	static function core_version_requied_value( $provider, $addon_file ) {
		...
		$constant = T4P_Pb_Addon::core_version_constant( $addon_file );
	...
	/**
	 * Check compatibility of this addon & T4P core
	 *
	 * @param string $core_required
	...
	public static function activation_hook() {
		do_action( 'reload_t4p_shortcodes' );
	}
	...
	public static function update_hook( $plugin_file ) {
		...
		$addon_version = T4P_Pb_Helper_Functions::get_plugin_info( $plugin_file, 'Version' );

		// Get stored version of addon
		$option_name = 't4p_addon_' . $addon_folder . '_version';
