...
if ( ! class_exists( 'T4P_Pb_Gadget_Base' ) ) :
...
class T4P_Pb_Gadget_Base {
	...
	public static function hook() {
		...
		if ( isset( $_REQUEST['t4p-gadget'] ) ) {
			// Prepare gadget action
			$action = isset( $_REQUEST['action'] ) ? $_REQUEST['action'] : null;

			if ( ! empty( $action ) ) {
				// Execute gadget action
				self::execute( $_REQUEST['t4p-gadget'], $action );
			}
		}
	}
	...
	protected static function get_instance( $gadget, $method = 'default_action' ) {
		...
		if ( ! isset( self::$_instance[ $gadget ] ) ) {
			...
			$class = 'T4P_Gadget_' . implode( '_', $class );
	...
	public function render( $action = 'default' ) {
		...
		$file = T4P_Pb_Loader::get_path( "gadget/tmpl/{$this->gadget}/{$action}.php" );
