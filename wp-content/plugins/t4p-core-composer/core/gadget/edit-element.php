 ...
 */
/**
 * Gadget class for loading editor for T4P PageBuilder element.
 *
 * @package  T4P_PageBuilder
 * @since    2.0.2
 */
class T4P_Gadget_Edit_Element extends T4P_Pb_Gadget_Base {
	/**
	...
	/**
	 * Load form for editing T4P Page Builder element.
	 *
	 * @return  void
	 */
	public function form_action() {
		...
		if ( isset( $_GET['t4p_shortcode_preview'] ) && 1 == $_GET['t4p_shortcode_preview'] ) {
			$t4p_pb->shortcode_iframe_preview();
		} else {
			$t4p_pb->modal_page_content();
		}
		...
		if ( $t4p_pb_preview_class != 'Widget' ) {
	...
	/**
	 * Load HTML code for inserting element into  T4P Page Builder area.
	 *
	 * @return  void
	 */
	public function insert_action() {
		...
		exit( T4P_Pb_Helper_Shortcode::do_shortcode_admin( $raw_shortcode ) );
	}
	...
	public function filter_assets() {
		...
		if ( ! isset( $executed ) ) {
			...
			$on_demand_assets = apply_filters( 't4p-edit-element-required-assets', $on_demand_assets );
	...
	protected function load_asset( $handle ) {
		...
		if ( preg_match( '/\.(css|js)$/', $handle ) ) {
			$handle = T4P_Pb_Init_Assets::file_to_handle( $handle );
		}

		// Load asset
		T4P_Pb_Init_Assets::load( $handle );
	}
}
