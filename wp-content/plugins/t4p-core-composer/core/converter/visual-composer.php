 ...
 * @since  2.3.0
 */
class T4P_Pb_Converter_Visual_Composer extends T4P_Pb_Converter {
	/**
	...
	protected $mapping = array(
		...
		't4p_item_carousel' => array(
			'attributes' => array(
				'image_file' => 'image_file',
			),
		),

		't4p_contentclips' => array(
			'attributes' => array(
				'title'                    => 'el_title',
				'size'                     => 't4p_cl_limit',
				'order_by'                 => 't4p_cl_orderby',
				'order'                    => 't4p_cl_order',
				'post_type'                => 't4p_cl_source',
				...
				'count'                    => 't4p_cl_limit',
				'posttypes'                => 't4p_cl_source',
				'categories'               => 'item_filter',
				'orderby'                  => 't4p_cl_orderby',
				'el_class'                 => 'css_suffix',
			),
		),

		't4p_item_progressbar' => array(
			'attributes' => array(
				'pbar_text'       => 'pbar_text',
	...
	public function __construct( $post ) {
		...
		add_filter( "t4p_pb_parse_data_{$this->converter}", array( &$this, 'prepare_visual_composer_data' ) );

		// Register filters to convert Visual Composer shortcodes
		add_filter( 't4p_pb_convert_vc_row_shortcode'            , array( &$this, 'convert_vc_row_shortcode'          ) );
		add_filter( 't4p_pb_convert_vc_column_shortcode'         , array( &$this, 'convert_vc_column_shortcode'       ) );
		add_filter( 't4p_pb_convert_vc_separator_shortcode'      , array( &$this, 'convert_vc_separator_shortcode'    ) );
		add_filter( 't4p_pb_convert_vc_text_separator_shortcode' , array( &$this, 'convert_vc_separator_shortcode'    ) );
		add_filter( 't4p_pb_convert_vc_single_image_shortcode'   , array( &$this, 'convert_vc_single_image_shortcode' ) );
		add_filter( 't4p_pb_convert_vc_gallery_shortcode'        , array( &$this, 'convert_vc_gallery_shortcode'      ) );
		add_filter( 't4p_pb_convert_vc_images_carousel_shortcode', array( &$this, 'convert_vc_gallery_shortcode'      ) );
		add_filter( 't4p_pb_convert_vc_tabs_shortcode'           , array( &$this, 'convert_vc_tabs_shortcode'         ) );
		add_filter( 't4p_pb_convert_vc_tour_shortcode'           , array( &$this, 'convert_vc_tabs_shortcode'         ) );
		add_filter( 't4p_pb_convert_vc_posts_grid_shortcode'     , array( &$this, 'convert_vc_posts_grid_shortcode'   ) );
		add_filter( 't4p_pb_convert_vc_carousel_shortcode'       , array( &$this, 'convert_vc_posts_grid_shortcode'   ) );
		add_filter( 't4p_pb_convert_vc_posts_slider_shortcode'   , array( &$this, 'convert_vc_posts_grid_shortcode'   ) );
		add_filter( 't4p_pb_convert_vc_button_shortcode'         , array( &$this, 'convert_vc_button_shortcode'       ) );
		add_filter( 't4p_pb_convert_vc_button2_shortcode'        , array( &$this, 'convert_vc_button_shortcode'       ) );
		add_filter( 't4p_pb_convert_vc_cta_button_shortcode'     , array( &$this, 'convert_vc_cta_button_shortcode'   ) );
		add_filter( 't4p_pb_convert_vc_cta_button2_shortcode'    , array( &$this, 'convert_vc_cta_button_shortcode'   ) );
		add_filter( 't4p_pb_convert_vc_video_shortcode'          , array( &$this, 'convert_vc_video_shortcode'        ) );
		add_filter( 't4p_pb_convert_vc_gmaps_shortcode'          , array( &$this, 'convert_vc_gmaps_shortcode'        ) );
		add_filter( 't4p_pb_convert_vc_raw_html_shortcode'       , array( &$this, 'convert_vc_raw_html_shortcode'     ) );
		add_filter( 't4p_pb_convert_vc_raw_js_shortcode'         , array( &$this, 'convert_vc_raw_html_shortcode'     ) );
		add_filter( 't4p_pb_convert_vc_progress_bar_shortcode'   , array( &$this, 'convert_vc_progress_bar_shortcode' ) );
		add_filter( 't4p_pb_convert_vc_pie_shortcode'            , array( &$this, 'convert_vc_pie_shortcode'          ) );

		// Register action to finalize data conversion
		add_action( "t4p_pb_after_convert_{$this->converter}_data", array( &$this, 'finalize' ) );
	}
	...
	public function prepare_visual_composer_data() {
		...
		// Clean-up all nested rows and columns because T4P PageBuilder does not support nested row
		...
		// Wrap continuous `vc_toggle` shortcode inside an T4P Accordion element
		...
		if ( 1 < count( $parts ) ) {
			...
			for ( $i = 1, $n = count( $parts ); $i < $n; $i++ ) {
				...
				if ( $first ) {
					$part = '[t4p_accordion multi_open="yes"]' . $part;

					$first = false;
				}

				if ( '[/vc_toggle]' != substr( $part, -12 ) ) {
					$part = str_replace( '[/vc_toggle]', '[/vc_toggle][/t4p_accordion]', $part );
		...
		foreach ( self::$widget_mapping as $tag => $widget ) {
			if ( false !== strpos( $this->post->post_content, '[' . $tag ) ) {
				$this->post->post_content = str_replace( '[' . $tag, '[t4p_widget widget_id="' . $widget . '"', $this->post->post_content );
			}
		}
	...
	public function convert_vc_gallery_shortcode( $element ) {
		...
		for ( $i = 0, $n = count( $images ); $i < $n; $i++ ) {
			if ( 1 < $element['attributes']['slides_per_view'] || ( isset( $element['attributes']['onclick'] ) && 'link_no' != $element['attributes']['onclick'] ) ) {
				...
				if ( (int) $element['attributes']['slides_per_view'] == count( $slides ) || $n == $i + 1 ) {
					// Create new T4P Item Carousel element
					$children[] = array(
						'tag'      => 't4p_item_carousel',
						'children' => $slides,
					);
				...
				// Create new `t4p_item_carousel` shortcode
				$children[] = array(
					'tag'        => 't4p_item_carousel',
		...
		// Create new T4P Carousel element
		$carousel = array(
			'tag'        => 't4p_carousel',
	...
	public function convert_vc_posts_grid_shortcode( $element ) {
		// Check if equivalent element available in T4P PageBuilder
		global $shortcode_tags;

		if ( ! array_key_exists( 't4p_contentclips', $shortcode_tags ) ) {
	...
	public function convert_vc_progress_bar_shortcode( $element ) {
		...
		if ( isset( $params['values'] ) && ! empty( $params['values'] ) ) {
			...
			foreach ( $children as & $item ) {
				...
				// Create new `t4p_item_progressbar` shortcode
				$item = array(
					'tag'        => 't4p_item_progressbar',
					'attributes' => array(
						'pbar_text'       => $text,
	...
	public function convert_vc_pie_shortcode( $element ) {
		...
		if ( ! array_key_exists( 't4p_progresscircle', $shortcode_tags ) ) {
			// Equivalent element not available, keep original shortcode as is
			return $element;
		}

		// Convert shortcode tag
		$element['tag'] = 't4p_progresscircle';
