 ...
 *
 * Base class for converting data from other page builder plugin to T4P PageBuilder.
 *
 * @since  2.3.0
 */
class T4P_Pb_Converter {
	/**
	...
	public static function get_converter( $converter, $post ) {
		...
		if ( ! isset( self::$_instance[ $converter ] ) ) {
			...
			$class = 'T4P_Pb_Converter_' . implode( '_', $class );
	...
	public static function get_converters() {
		...
		$converters = apply_filters( 't4p_pb_get_data_converters', $converters );
	...
	/**
	 * Parse data of other page builder from given post then convert to T4P PageBuilder data.
	 *
	...
	public function convert() {
		...
		$data    = apply_filters( "t4p_pb_parse_data_{$this->converter}", $data   , $this );
		$mapping = apply_filters( "t4p_pb_map_data_{$this->converter}"  , $mapping, $this );

		// Check if we have data to do conversion
		if ( is_string( $data ) && empty( $data ) ) {
			return sprintf(
				__( 'Not found any %s data.', 't4p-core' ),
		...
		// Convert parsed data to T4P PageBuilder data
		...
		if ( isset( $_REQUEST['backup_data'] ) && $_REQUEST['backup_data'] ) {
			...
			if ( ! ( $id = wp_insert_post( $backup ) ) ) {
				return __( 'Cannot backup current post.', 't4p-core' );
			}
		...
		if ( ! wp_insert_post( $post ) ) {
			return __( 'Cannot update current post.', 't4p-core' );
		}

		// Update post meta to activate T4P PageBuilder tab
		update_post_meta( $this->post->ID, '_t4p_page_active_tab', 1 );

		// Store T4P PageBuilder data to post meta also
		if ( ! update_post_meta( $this->post->ID, '_t4p_page_builder_content', $data ) ) {
			return __( 'Cannot store T4P PageBuilder data to post meta.', 't4p-core' );
		}

		// Trigger action to finalize data conversion
		do_action( "t4p_pb_after_convert_{$this->converter}_data", $post, $this->post );

		return $post['ID'];
	}

	/**
	 * Process matched shortcode to array of data that is able to convert to T4P PageBuilder shortcode later.
	 *
	...
	/**
	 * Convert parsed data of other page builder elements to T4P PageBuilder elements.
	 *
	...
	protected function convert_elements( $elements, $mapping ) {
		...
		if ( is_array( $elements ) && is_array( $mapping ) ) {
			// Map parsed data to T4P PageBuilder data
			foreach ( $elements as $element ) {
				// Allow shortcode filterable before doing conversion
				$element = apply_filters( "t4p_pb_convert_{$element['tag']}_shortcode", $element );
				...
				if ( isset( $mapping[ $element['tag'] ] ) && is_array( $mapping[ $element['tag'] ] ) ) {
					...
					// Prepare children for T4P Widget shortcode
					if ( 'widget' == $name ) {
						$children = $this->to_query_string( $element['attributes'], '', array( 'widget_id' ) );
					}
				} else {
					// There is no mapping available for this shortcode, wrap it inside an T4P Text element
					$name = 't4p_text';
