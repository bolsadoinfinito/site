 ...
 * @since  2.3.0
 */
class T4P_Pb_Converter_Site_Origin extends T4P_Pb_Converter {
	/**
	...
	protected $mapping = array(
		...
		't4p_testimonial' => array(),

		't4p_item_testimonial' => array(
			'attributes' => array(
				'name'  => 'name',
				'image' => 'image_file',
			),
	...
	public function __construct( $post ) {
		...
		add_filter( "t4p_pb_parse_data_{$this->converter}", array( &$this, 'parse_site_origin_data' ) );
	...
	/**
	 * Parse Site Origin data from given post then convert to T4P PageBuilder data.
	 *
	...
	protected function parse_siteorigin_panels_widget_testimonial( $widget, $class = '' ) {
		...
		if ( ! array_key_exists( 't4p_testimonial', $shortcode_tags ) ) {
			// Equivalent element not available, embed original widget directly
			return $this->parse_widget( $widget, $class );
		}
