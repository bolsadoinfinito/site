 ...
 */
if ( ! class_exists( 'T4P_Pb_Helper_Layout' ) ) {

	class T4P_Pb_Helper_Layout {
		...
		static function save_premade_layouts( $layout_name, $layout_content ) {
			$error      = 0;
			$upload_dir = T4P_Pb_Helper_Functions::get_wp_upload_folder( '/t4p-pb-layout/' . T4P_PAGEBUILDER_USER_LAYOUT );
			...
			if ( file_exists( $file ) ) {
				...
				$fp = fopen( $file, 'w' );
				fwrite( $fp, '[t4p_layout name="' . $layout_name . '"]' );
		...
		static function get_premade_layouts() {
			$path = T4P_Pb_Helper_Functions::get_wp_upload_folder( '/t4p-pb-layout/' );
		...
		static function get_uri( $dir, $file ) {
			if ( $dir == T4P_PB_PREMADE_LAYOUT ) {
				$uri = T4P_PB_PREMADE_LAYOUT_URI;
			} else {
				$path_parts = pathinfo( $dir );
				$uri        = T4P_Pb_Helper_Functions::get_wp_upload_url( '/t4p-pb-layout/' ) . $path_parts['basename'];
			}
		...
		static function extract_layout_data( $path, $data ) {
			...
			if ( $fp ) {
				...
				$pattern  = '/\[t4p_layout\s+([A-Za-z0-9_-]+=\"[^"\']*\"\s*)*\s*\]/';
				fclose( $fp );
		...
		static function get_provider_info( $dir, $info = 'name' ) {
			if ( $info == 'id' ) {
				...
				return ( ( $dir == T4P_PB_PREMADE_LAYOUT ) ? 't4p_pb' : $path_parts['basename'] );
			}
			if ( $dir == T4P_PB_PREMADE_LAYOUT ) {
				return __( 'T4P Templates', 't4p-core' );
			}
			...
			return __( 'Your Templates', 't4p-core' );
		}
		...
		static function import( $dir ) {
			...
			$folder_to_create = ( $provider_name == __( 'Your Templates', 't4p-core' ) ) ? T4P_PAGEBUILDER_USER_LAYOUT : sanitize_title( $provider_name );
			$new_dir          = T4P_Pb_Helper_Functions::get_wp_upload_folder( '/t4p-pb-layout/' . $folder_to_create, false );
		...
		static function remove_group( $group ) {
			...
			$dir   = T4P_Pb_Helper_Functions::get_wp_upload_folder( '/t4p-pb-layout/' . $group, false );
			if ( is_dir( $dir ) ) {
				T4P_Pb_Utils_Common::recursive_delete( $dir );
