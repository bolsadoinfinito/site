 ...
 * @version        1.0.0
 */
class T4P_Pb_Helper_Html_Group_Table extends T4P_Pb_Helper_Html {
	/**
	 * Group items
	 *
	 * @param type $element
	...
	static function render( $element ) {
		...
		if ( $sub_items ) {
			foreach ( $sub_items as $idx => $item ) {
				...
				$shortcode_data = '';
				if ( ! $label_item ) {
					$content = __( $shortcode_name, 't4p-core' ) . ' ' . __( 'Item', 't4p-core' ) . ' ' . ( $idx + 1 );
				} else {
				...
				if ( ! empty( $item['std'] ) ) {
					...
					$params         = T4P_Pb_Helper_Shortcode::generate_shortcode_params( $el->items, NULL, $extract_params, TRUE, FALSE, $content );
					$el->shortcode_data();
					$params['extract_title'] = empty ( $params['extract_title'] ) ? __( '(Untitled)', 't4p-core' ) : $params['extract_title'];
		...
		$element_name = ( isset( $_element['name'] ) ) ? $_element['name'] : __( ucwords( ( ! $label_item ) ? $shortcode_name : $label_item ), 't4p-core' ) . ' ' . __( 'Items', 't4p-core' );
