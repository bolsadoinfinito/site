 ...
 * @version        1.0.0
 */
class T4P_Pb_Helper_Html_Checkbox extends T4P_Pb_Helper_Html {
	/**
	 * Checkbox option
	 * @param type $element
	 * @return type
	...
	static function render( $element ) {
		...
		$_t4p_has_depend = ( ! empty($element['has_depend'] ) && $element['has_depend'] == '1') ? ' t4p_has_depend' : '';
		...
		foreach ( $element['options'] as $key => $text ) {
			...
			$str = "<label class='" . $_class . "'><input class='{$_t4p_has_depend}{$add_class}' type='" . $type . "' value='$key' id='{$element['id']}' name='{$element['id']}' $checked DATA_INFO $action_item/>$text</label>$linebreak";
