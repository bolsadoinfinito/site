 ...
 * @version        1.0.0
 */
class T4P_Pb_Helper_Html_Margin extends T4P_Pb_Helper_Html {
	/**
	...
	static function render( $element, $input_params ) {
		...
		if ( in_array( 't', $element['margin_elements'] ) ) {
			...
			$_top = array(
				...
				'append_before' => '<i class="input-mini t4p-label-prefix">' . __( 'Top', 't4p-core' ) . '</i>',
				'append' => 'px',
				'validate' => 'number',
				'bound' => '0',
			);
			$output .= T4P_Pb_Helper_Shortcode::render_parameter( 'text_append', $_top );
		}
		...
		if ( in_array( 'l', $element['margin_elements'] ) ) {
			...
			$_left = array(
				...
				'append_before' => '<i class="input-mini t4p-label-prefix">' . __( 'Left', 't4p-core' ) . '</i>',
				'append' => 'px',
				'validate' => 'number',
				'bound' => '0',
			);
			$output .= T4P_Pb_Helper_Shortcode::render_parameter( 'text_append', $_left );
		}
		...
		if ( in_array( 'b', $element['margin_elements'] ) ) {
			...
			$_bottom = array(
				...
				'append_before' => '<i class="input-mini t4p-label-prefix">' . __( 'Bottom', 't4p-core' ) . '</i>',
				'append' => 'px',
				'validate' => 'number',
				'bound' => '0',
			);
			$output .= T4P_Pb_Helper_Shortcode::render_parameter( 'text_append', $_bottom );
		}
		...
		if ( in_array( 'r', $element['margin_elements'] ) ) {
			...
			$_right = array(
				...
				'append_before' => '<i class="input-mini t4p-label-prefix">' . __( 'Right', 't4p-core' ) . '</i>',
				'append' => 'px',
				'validate' => 'number',
				'bound' => '0',
			);
			$output .= T4P_Pb_Helper_Shortcode::render_parameter( 'text_append', $_right );
		}
