...
if ( ! class_exists( 'T4P_Pb_Helper_Html' ) ) {

	class T4P_Pb_Helper_Html {
		...
		static function get_dependency( $element ) {
			...
			if ( $dependency ) {
				...
				$depend_info['class'] = ' t4p_hidden_depend t4p_depend_other ';
			}
			return $depend_info;
		}
		...
		static function get_extra_info( $element ) {
			...
			$element['class'] .= ( ! empty($element['has_depend'] ) && $element['has_depend'] == '1') ? ' t4p_has_depend' : '';
		...
		static function get_style( $element, $output ) {
			...
			$styles = '';
			$output  = T4P_Pb_Utils_Placeholder::remove_placeholder( $output, 'custom_style', $styles );
			return $output;
		}
		...
		static function get_label( $element ) {
			...
			if ( ( ! isset( $element['showlabel'] ) || $element['showlabel'] != '0' ) && isset( $element['name'] ) ) {
				...
				if ( isset( $element['tooltip'] ) ) {
					$tooltip = '<i class="icon-help t4p-tooltip-toggle" data-html="true" title = "' . $element['tooltip'] . '"></i>';
				}

				$label = "<label class='control-label t4p-label-left'><span >{$element['name']}</span>{$tooltip}</label>";
			}
