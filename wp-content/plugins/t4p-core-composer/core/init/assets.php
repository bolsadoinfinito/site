...
if ( ! class_exists( 'T4P_Pb_Init_Assets' ) ) :
...
class T4P_Pb_Init_Assets {
	 ...
	 */
		't4p-pb-addons-css' => array(
			'src' => 'assets/theme4press/css/addons.css',
			'deps' => array( 't4p-pb-bootstrap-css', 't4p-jquery-ui-css' ),
	),

		't4p-pb-addons-js' => array(
			'src' => 'assets/theme4press/js/addons.js',
			'deps' => array( 't4p-pb-bootstrap-js', 'jquery-ui-dialog' ),
	),
	);
	...
	public static function hook() {
		...
		if ( ! isset( $registered ) ) {
			// Admin or frontend?
			$prefix = apply_filters( 't4p_pb_asset_hook_prefix', defined( 'WP_ADMIN' ) ? 'admin' : 'wp' );
			...
			add_filter( 't4p_pb_register_assets', array( __CLASS__, 'prepare' ), 1000 );
	...
	public static function footer() {
		...
		self::$localize = apply_filters( 't4p_pb_localize_assets', self::$localize );
	...
	protected static function register() {
		...
		self::$assets = apply_filters( 't4p_pb_register_assets', self::$assets );
	 ...
	 * @return  string
	 */
	public static function file_to_handle( $asset, $prefix = 't4p-' ) {
