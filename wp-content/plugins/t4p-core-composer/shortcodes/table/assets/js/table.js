...
( function ($)
{
	...
	$.IGTable.prototype = {
        ...
        spanProcess: function(parent_wrap){
            ...
            var data = {};
            var error = 0;
            var extract_data = $( '#jsn_view_modal').find('#t4p_extract_data');
            ...
            $.each(extract_data.text().split('#'), function(key, value){
                if(value){
                    ...
                    if(attr_value <= 0){
                        alert(t4p_translate.table.table3);
                        error = 1;
                    }
            ...
            var max_col = table.find('tr.t4p-row-of-delete').find('td').length;
            ...
            table.find('.t4p-remove-cell').each(function(){
                $(this).remove();
            });

            // reset #t4p_extract_data value
            extract_data.html('');
            self.reindexTable();
            self.cleanDeleteBtn();
        },

        // get cell content and add 't4p-remove-cell' class
        cellProcess:function(this_, self_content){
            ...
            this_.addClass('t4p-remove-cell');
        },
        ...
        cleanDeleteBtn:function(){
            $("#table_content tr").each(function(row_idx){
                ...
                if($(this).find(child).length == 1)
                    $(this).find('.t4p-delete-column-td').empty();
            })
        },
            ...
            switch (type) {
                    ...
                    case 'row':
                        ...
                        if($("#table_content tr").length == 3){
                            alert(t4p_translate.table.table2);
                            return true;
                        }
    ...
    var tr_start = "<textarea class='hidden' data-sc-info='shortcode_content' name='shortcode_content[]'>[item_table tagname='tr_start' ][/item_table]</textarea>";
    var tr_end = "<textarea class='hidden' data-sc-info='shortcode_content' name='shortcode_content[]'>[item_table tagname='tr_end' ][/item_table]</textarea>";
    $(document).ready(function(){
        var t4p_table = new $.IGTable();
        ...
        $("#table_content").find("tr").slice(0, $("#table_content").find("tr").length - 1).each(function(i){
            ...
            t4p_table.appendTextarea($(this), first_child);
        })

        t4p_table.addRowCol();
        t4p_table.reindexTable();

		$('body').on('init_table_sc', function(e, active_shortcode){
			t4p_table.init(active_shortcode);
		});
    })
