 ...
 * @since    2.1.0
 */
class Button extends T4P_Pb_Shortcode_Element {
	...
	public function element_config() {
		...
		$this->config['name']        = __( 'Button', 't4p-core' );
		$this->config['cat']         = __( 'Typography', 't4p-core' );
		$this->config['icon']        = 't4p-pb-icon-button';
		$this->config['description'] = __( 'Eye catching button for hyperlinks', 't4p-core' );

		// Define exception for this shortcode
                $this->config['exception'] = array(
			'default_content'  => __( 'Button', 't4p-core' ),
			'data-modal-title' => __( 'Button', 't4p-core' ),
                );
		...
		if ( strtolower( $align ) != 'inherit' ) {
			...
			if ( strtolower( $align ) == 'right' )
                ...
                if ( $icon && $icon_divider == 'yes' ) {
                    $span = "<span class='t4p-button-text-{$icon_position}' >{$content}</span>";
                } else {
                    $span = "<span class='t4p-button-text' >{$content}</span>";
                }
                ...
                $html_element = "{$styles}<a class='btn-default button {$cls_alignment} {$size} button {$color} t4p-button button-{$type} button-{$shape} button-{$size} button-{$color} button-$this->button_counter buttonshadow-{$shadow}' type='message/http' {$modaltype} {$href} {$target} {$title}>{$inner_html}</a>{$clerfix}";
                
                if( $align == 'center' ) {
			$html_element = "<div class='button-align-center'>$html_element</div>";
		}
