...
function evolve_migrate_options() {
    ...
    if (!isset($data['bootstrap_slides']) || ( isset($data['bootstrap_slides']) && empty($data['bootstrap_slides']) )) {
        $hasSlides = true;
        if (!isset($data['bootstrap_slides'])) {
            $data['bootstrap_slides'] = array();
        }
        for ($i = 1; $i <= 5; $i ++) {
...
//            'type'     => 'palette',
//            'title'    => __( 'Palette Color Option', 'evolve' ),
//            'subtitle' => __( 'Only color validation can be done on this field type', 'evolve' ),
//            'default'  => 'color_palette_one',
//            'palettes' => array(
//                'color_palette_one'  => array(
//                    '#333',
//                    '#0bb697',
//                    '#273039',
//                    '#efefef',
//                ),
//                'color_palette_two' => array(
//                    '#6ed3cf',
//                    '#9068be',
//                    '#e1e8f0',
//                    '#e62739',
...
foreach ($slide_defaults as $k => $v) {
    $i = $k + 1;
    $slide_defaults[$k]['title'] = sprintf(__('Slide %s', 'evolve'), $i);
    $slide_defaults[$k]['image'] = array('url' => "{$evolve_imagepathfolder}bootstrap-slider/$i.jpg");
    $slide_defaults[$k]['enabled'] = 1;
}
...
Redux::setSection($evolve_opt_name, array(
    ...
    'fields' => array(
        array(
            ...
            'group_values' => true,
            // Group all fields below within the repeater ID
            'item_name' => __('Slide', 'evolve'),
            // Add a repeater block name to the Add and Delete buttons
            'bind_title' => 'title',
            // Bind the repeater block title to this field ID
            //'forced_collapse'=> false, // Only one slide can be open at once
            //'collapsible' => false, // One slide will always be open if set to false
            //'static'       => 5, // Set the number of repeater blocks to be output
